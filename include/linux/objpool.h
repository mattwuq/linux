 ...
 * 64bit:
 *        4     8      12     16        32                 64
 * | head | head | tail | tail | ages[4] | ents[4]: (8 * 4) | objects
 *
 * 32bit:
 *        4      8     12     16        32        48       64
 * | head | tail | ages[4] | ents[4] | unused | objects ...
 *
 */

struct objpool_slot {
	local_t                head;	/* head of ring array */
	local_t                tail;	/* tail of ring array */
} __packed;
...
struct objpool_head {
	...
	int                     capacity;	/* max objects per cpuslot, pow of 2 */
	int            		slot_bits;	/* log2 of capacity */
	unsigned int            slot_mask;	/* capacity - 1 */
...
/* release whole objpool forcely (all objs to be released) */
void objpool_free(struct objpool_head *head);

/* cleanup all objects and defref objpool for releasing */
